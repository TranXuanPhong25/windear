...
export default function DropZone({onDropFile, editingBookImageUrl}: {
    ...
    editingBookImageUrl: string | null
}) {
    const [image, setImage] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string|null>(editingBookImageUrl);
    ...
    useEffect(() => {
        if (editingBookImageUrl === null) {
            setImage(null);
            setPreviewUrl(null);
        } else if (editingBookImageUrl != "") {
    ...
    return (
        ...
        >
                    ...
                    (image || previewUrl) ? (
                        ...
                        : (
                           ...
                           <em>(Only *.jpeg and *.png images will be accepted)</em> */}
                                <div className="">
                                    <PlusCircle className="size-10 mx-auto opacity-60 hover:opacity-100 cursor-pointer"
                                                onClick={open}/>
                                    <p>Click to upload</p>
            ...
            <div
                className="absolute transition-all top-[55%] left-1/2 -translate-x-1/2 group-hover:top-1/2 opacity-0 group-hover:opacity-100 -translate-y-1/2">
                {
                    (image || previewUrl) && (
                        <div className="flex items-center gap-3 ">
                            <div
                                className="bg-sky-500/70 text-white grid place-content-center size-10 rounded-full cursor-pointer hover:bg-sky-500"
                                onClick={open}>
                                <Pencil className="size-5"/>
                            </div>
                            <div className="bg-red-500/70 !text-white rounded-full p-2 hover:bg-red-500 cursor-pointer"
