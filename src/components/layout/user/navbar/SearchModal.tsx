...
const SearchModal: React.FC<SearchModalProps> = ({isOpen, onClose}) => {
    ...
    return (
        <div ref={overlayRef}
             className="fixed inset-0 z-50 flex justify-center  bg-opacity-80 bg-black dark:text-white ">
            <div ref={modalRef}
                 className="dark:bg-gray-800/90 rounded-2xl shadow-lg  w-full max-w-2xl h-fit mt-10 bg-slate-100 backdrop-blur-md border-gray-300/50 border-2 m-5">
                <div className="flex justify-between items-center relative mx-4 mt-6">
                    ...
                    <Button onClick={onClose} variant="ghost"
                            className="text-gray-500 hover:text-gray-700 dark:hover:bg text-md absolute right-2 h-0 px-3 py-4">
                        Esc
                    </Button>
                </div>
                <div className='relative mt-6'>

                    {
                        isLoading ? (
                            ...
                            <ScrollArea
                                className={clsx("w-full max-h-[50vh] border-gray-400/40 border-t-2", searchResults && searchResults.length && "h-[50vh]")}>
                                {
                                            ...
                                            result && (
                                                <div key={result.node.legacyId}
                                                    ...
                                                    <div className='mr-4 min-w-32 w-32 '>
                                                        <img
                                                            src={result.node.imageUrl || "/book-cover-unavailable-placeholder.jpg"}
                                                            alt={result.node.title}
                                                            className=" w-32 rounded-r-xl rounded-l-sm"/>
                                                    </div>
                                                    <Link to={`/books/${result.node.legacyId}`}
                                                          onClick={onClose}
                                                                ...
                                                                <p className='flex items-center'>
                                                                    {result.node.stats.averageRating != null ? (
                                                                            <>
                                                                                ...
                                                                                <Star
                                                                                    className="h-4 w-4 text-yellow-500 inline-block"
                                                                                    fill="rgb(234,179,8)"/>
                                                                            </>
                                                                        ) :
